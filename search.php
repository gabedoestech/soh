<?php
	...
	function doSearch($city, $first, $last, $app_name, $priceLow, $priceHigh, $specialty, $date, $sex, $sort)
	{
		...
		if ($first == "" && $sex == "" && $specialty == "")
			$stmt = queryMysql("SELECT A.*, D.specialty, U.userEmail, U.firstName, U.lastName, U.phone_no FROM
													appointment A, doctor D, users U WHERE A.city = '$city' AND A.taken = '0'
													AND A.app_date >= '$curr' AND D.userID = A.userID AND U.userID = D.userID ORDER BY $sort");
		elseif ($first == "" && $sex == "")
		{
