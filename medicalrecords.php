<?php
  ...
  $patient_name = $row2['firstName'] . " " . $row2['lastName'];
  ...
  if ($user_home->is_doctor())
    $query3 = queryMysql("SELECT A.*, D.specialty, U.userEmail, U.firstName, U.lastName, U.phone_no FROM appointment A, users U, doctor D, sees S
                          ...
                          AND A.appointment_id=S.appointment_id AND D.specialty = (SELECT D.specialty FROM doctor D WHERE userID = '$userID')");
  else
    $query3 = queryMysql("SELECT A.*, D.specialty, U.userEmail, U.firstName, U.lastName, U.phone_no FROM appointment A, users U, doctor D, sees S
                          WHERE U.userID = A.userID AND D.userID = U.userID AND D.userID=S.userID_doctor AND S.userID_patient='$id'
  ...
  $result = queryMysql("SELECT P1.* FROM prescription P1, prescribes P2
                        WHERE P1.drugId=P2.drugId AND P2.userID_patient = '$id' ORDER BY P1.startDate ASC");
        ...
        <div class="container">
            ...
            <ol class="breadcrumb" id="bc1">
                ...
                <div class="panel panel-default">
                  <div id="HTMLtoPDF">
                    <?php echo "<div class='panel-heading' id='ph'><b>Medical Records for $patient_name</b></div>";?>
                ...
                <div class="panel panel-default">
                    ...
                    <div style="padding:15px">
                    	<!-- Added -->
                      <div align="right" onclick="HTMLtoPDF()"><li><a href="#"><button class="btn btn-medium btn-info"><b>Download PDF</b></button></right></a></li>
                                    </div>
			<!-- Added --><br>
                    <div align='right'>
                    <?php
                      if(!$user_home->is_doctor())
                      {
                        echo "<li><a href='medicalhistoryfillin.php?id=$id'><button class='btn btn-medium btn-info'><b>Edit Medical History</b></button></right></a></li>";
                    ?>
                    </div>
                    <?php }?>
                    </div>
